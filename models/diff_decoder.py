...
class DiffDecoderLayer(DecoderLayer):
    """Decoder layer with FiLM conditioning for diffusion"""
...
class DiffDecoder(nn.Module):
    """Diffusion decoder with FiLM conditioning"""
    def __init__(self, layers, emb_dim, nhead, head_dim, cond_dim, **kwargs) -> None:
        super().__init__()
        self.layers = nn.ModuleList([
            DiffDecoderLayer(emb_dim, nhead, head_dim, cond_dim, activation=geglu, **kwargs)
...
class DiffTransformer(nn.Module):
    """Diffusion transformer with optional context encoder"""
    def __init__(self, emb_dim, nhead, head_dim, cond_dim, num_encoder_layers,
                 num_decoder_layers, with_context=False, **kwargs) -> None:
        super().__init__()
        self.encoder = Encoder(num_encoder_layers, emb_dim, nhead, head_dim, **kwargs)
        self.decoder = DiffDecoder(num_decoder_layers, emb_dim, nhead, head_dim, cond_dim, **kwargs)
        self.with_context = with_context

        if with_context:
            self.context_encoder = Encoder(num_encoder_layers, emb_dim, nhead, head_dim, **kwargs)

        for p in self.parameters():
            if p.dim() > 1:
                xavier_uniform_(p)

    def forward(self, src, tgt, cond, ctx=None, dropout_mask=None, src_mask=None, tgt_mask=None,
                ctx_mask=None, memory_mask=None, src_key_padding_mask=None, ctx_key_padding_mask=None,
                tgt_key_padding_mask=None, memory_key_padding_mask=None):
        ...
        if dropout_mask is not None:
            ...
            if not dropout_mask.all():
                ...
                if ctx is not None and self.with_context:
...
class MIDI2SpecDiff(nn.Module):
    def __init__(self,
                 ...
                 emb_dim, nhead, head_dim, num_encoder_layers, num_decoder_layers,
                 with_context=False, **kwargs) -> None:
        super().__init__()
        ...
        self.transformer = DiffTransformer(
            emb_dim, nhead, head_dim, emb_dim * 4, num_encoder_layers, num_decoder_layers,
    ...
    def forward(self, midi_tokens, spec, t, context=None, **kwargs):
        ...
        if context is not None:
            context = self.linear_in(context) + self.out_pos_emb[:context.shape[1]]
